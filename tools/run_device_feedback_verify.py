"""Run device-feedback regressions serially in an isolated project copy.

Usage: python tools/run_device_feedback_verify.py --render
Covers map, card labels, rewards, altar and automatic turn ending.
"""
import argparse
import os
...
def main():
    ...
    parser.add_argument('--stage', choices=('feedback', 'regression', 'all'), default='all')
    args = parser.parse_args()
    root = Path(__file__).resolve().parents[1]
    sandbox = root / 'Temp/device-feedback-verify'
    ...
    def run(name, options, marker=''):
        ...
        if 'Failed to read the root certificate store.' in output:
            print(name + ': Windows certificate-store warning (offline UI tests)', flush=True)
        if result.returncode or errors or '[FAIL]' in output or marker and marker not in output:
    ...
    suites = {
        'feedback': [('device-feedback', 'verify/DeviceFeedbackVerify', 'DEVICE_FEEDBACK_RESULT:PASS')],
        'regression': [('card-queue', 'verify/CardPlayQueueVerify', 'CARD_QUEUE_RESULT:PASS'),
                       ('card-presentation', 'verify/CardPresentationVerify', 'CARD_PRESENTATION_RESULT:PASS')],
    }
