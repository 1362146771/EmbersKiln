"""Serial hidden act, combat and progression verification in an isolated Godot project."""
import argparse
import os
...
def main():
    ...
    sandbox = root / 'Temp/hidden-act-verify'
    ...
    run('hidden-act', opts + ['--quit-after', '7200', 'res://scenes/verify/HiddenActVerify.tscn'] + (['--', '--visual'] if args.render else []), 'HIDDEN_ACT_RESULT:PASS')
    ...
    run('card-pool', ['--headless', '--quit-after', '2400', 'res://scenes/verify/IroncladCardPoolVerify.tscn'], 'IRONCLAD_CARD_POOL_RESULT:PASS')
    run('pre-run-revive', ['--headless', '--quit-after', '2400', 'res://scenes/verify/PreRunReviveVerify.tscn'], 'PRE_RUN_REVIVE_RESULT:PASS')
    run('card-queue', opts + ['--quit-after', '3600', 'res://scenes/verify/CardPlayQueueVerify.tscn'], 'CARD_QUEUE_RESULT:PASS')
    if args.render:
        output = root/'outputs/qa/hidden-act'
        output.mkdir(parents=True, exist_ok=True)
        for image in (sandbox/'Temp').glob('hidden_*.png'):
            shutil.copy2(image, output/image.name)
    print('HIDDEN_ACT_SUITE:PASS', flush=True)
